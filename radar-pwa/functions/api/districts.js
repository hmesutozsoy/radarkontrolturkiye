...
export async function onRequest({ request }) {
  ...
  if (!cityId) return new Response(JSON.stringify({ error: "cityId required" }), { status: 400, headers: { "Content-Type": "application/json", ...cors } });
  try {
    const resp = await fetch(`${ICISLERI_BASE}/GetDistricts?cityId=${cityId}`, {
